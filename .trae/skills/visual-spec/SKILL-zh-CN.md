...
2. 确保 `/test/` 存在，并确保子目录存在：
   ...
   - `/test/集成测试/`
   - `/test/playwright/`
3. 按 `/scheme.yaml` 的 `selected.language` 复制内置用例阅读器到 `/test/testcase_reader.html`（覆盖写入）：
   - `en` → `prompts/vspec_testcase_reader/testcase_reader.en-US.html`（若不存在则回退到 `prompts/vspec_testcase_reader/testcase_reader.html`）
