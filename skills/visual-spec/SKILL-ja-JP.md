...
機能一覧に基づき要件詳細を展開します（`/specs/details/` に生成）。各ステップの種別（端=Web/Mobile/Web+Mobile/Backend/Job）を判定し、対応するロジック詳細を必ず生成します（ロジックを省略しない）。あわせて、Markdown/PlantUML をレンダリングできる閲覧用 `reader.html` を `/specs/details/reader.html` に生成します。
