...
0.2 若用户在命令参数中传入 `lang=<en|zh|ja>`，则将 `/scheme.yaml` 的 `selected.language` 设置为该值（只更新该字段，其他字段与格式保持不变；`zh-CN` 视为 `zh` 的别名）。
...
用于生成访谈用户的大纲与调查问卷（Markdown），覆盖系统建设目的、目标、用户群体、场景、要解决的问题、使用频率等关键问题，并以“判断题/选择题”为主。

流程：
1. 加载 `prompts/vspec_interview/interview.md`。
2. 生成问卷并写入：`/docs/current/interview_questionnaire.md`。

### `/vspec:i-word`

用于生成调查问卷的 Word 版本（Word 可打开的单文件 `.docx`，HTML 格式）。

流程：
1. 若 `/docs/current/interview_questionnaire.md` 不存在：停止并提示先运行 `/vspec:interview`。
2. 加载 `prompts/vspec_interview/interview_word.md`。
3. 将输出写入：`/docs/current/interview_questionnaire.docx`。
...
5. 加载 `prompts/vspec_detail/index.md` 生成单页阅读入口：`/specs/details/reader.html`
...
6. 加载 `prompts/harness/verify/post_verify_stack_verify.md` 检查原型前端工程是否严格符合 `/scheme.yaml` 中选择的前端栈；若输出了问题列表，则提示问题并立即结束。
...
10. 加载 `prompts/harness/verify/post_verify_mobile_selection_check.md` 检查移动端数据选择是否使用“选择页（List）选择→返回回填”，而不是下拉框 Select；若输出了问题列表，则提示问题并立即结束。
11. 加载 `prompts/harness/verify/post_verify_price_format_check.md` 检查价格/金额格式是否统一（右对齐、两位小数、千位分隔符）；若输出了问题列表，则提示问题并立即结束。
12. 加载 `prompts/harness/verify/post_verify_click_check.md` 检查是否存在“按钮/链接点击无反应”的情况；若输出了问题列表，则提示问题并立即结束。
13. 加载 `prompts/harness/verify/post_verify_verify.md` 检查原型是否覆盖关键约束（移动端/审批/CRUD/布局/登录/RBAC 等）。若输出了问题列表，则提示问题并立即结束。
...
2. 确保 `/test/` 存在，并确保子目录存在：
   - `/test/验收用例/`
   - `/test/单元测试/`
   - `/test/集成测试/`
   - `/test/playwright/`
3. 复制内置用例阅读器 `prompts/vspec_testcase_reader/testcase_reader.html` 到 `/test/testcase_reader.html`（覆盖写入）。该模板已内置中日英三语切换功能。
4. 加载 `prompts/vspec_accept/accept.md` 生成“场景驱动”的验收用例（JSON），覆盖主流程、异常、边界、权限与数据范围。
5. 将验收用例写入 `/test/验收用例/acceptance_cases.json`。

### `/vspec:i-test`

用于生成单元测试与集成测试用例（JSON），用例从场景与可实现粒度的细节规格中推导。

语言：
- 读取 `/scheme.yaml` 的 `selected.language`（支持 `en`、`zh`、`ja`；若缺失/非法则按 `en` 处理）。
- JSON 中所有用例标题与描述必须统一使用所选语言。

流程：
1. 读取 `/specs/functions/*`、`/specs/background/scenarios.md`、`/specs/background/scenario_details/`、`/specs/background/roles.md`、`/specs/details/`、`/specs/models/*.md`（如有）。
2. 确保 `/test/` 存在，并确保子目录存在：
   - `/test/单元测试/`
   - `/test/集成测试/`
3. 复制内置用例阅读器 `prompts/vspec_testcase_reader/testcase_reader.html` 到 `/test/testcase_reader.html`（覆盖写入）。该模板已内置中日英三语切换功能。
4. 加载 `prompts/vspec_i_test/i_test.md` 生成单元测试用例（JSON），写入 `/test/单元测试/unit_test_cases.json`。
5. 再次加载 `prompts/vspec_i_test/i_test.md` 生成集成测试用例（JSON），写入 `/test/集成测试/integration_test_cases.json`。

### `/vspec:script`

用于基于 JSON 测试用例生成 Playwright 脚本。

流程：
1. 读取以下 JSON 用例（存在则读取）：
   - `/test/验收用例/acceptance_cases.json`
   - `/test/集成测试/integration_test_cases.json`
2. 确保 `/test/playwright/` 存在。
3. 加载 `prompts/vspec_script/script.md` 生成 Playwright 测试脚本。
4. 写入输出：
...
流程：
1. 读取 `/test/验收用例/acceptance_cases.json`、`/specs/functions/*`、`/specs/details/`，并识别仓库中已有的测试框架。
2. 加载 `prompts/vspec_test/test.md`，按既有框架与约定生成自动化测试。
3. 将测试代码写入项目测试目录（若不存在标准目录则写入 `/tests/`），并确保能通过已有脚本运行。
4. 加载 `prompts/harness/append-test/post_append_test_coverage_check.md` 检查测试覆盖率是否足够完整；若输出了问题列表，则继续执行下一步。
...
4. 加载 `prompts/harness/impl/post_impl_verify.md` 检查后端是否符合 MVC 架构且测试覆盖是否完整；若输出了问题列表，则提示问题并立即结束。
...
- `prompts/vspec_interview/interview.md`：`/vspec:interview` 使用的提示词，用于生成 `/docs/current/interview_questionnaire.md`。
- `prompts/vspec_interview/interview_word.md`：`/vspec:i-word` 使用的提示词，用于生成 `/docs/current/interview_questionnaire.docx`（Word 可打开的单文件 HTML）。
...
- `prompts/vspec_detail/index.md`：生成 `/specs/details/reader.html` 单页阅读入口（Markdown/PlantUML 渲染）的提示词。
- `prompts/vspec_detail/index.html`：`/specs/details/reader.html` 的固定 HTML 模板（用于稳定生成：目录树 + Markdown 渲染）。
...
- `prompts/vspec_detail/message_queue.md`：生成消息队列设计的提示词。
- `prompts/vspec_detail/cache.md`：生成缓存设计的提示词。
...
- `prompts/vspec_i_test/i_test.md`：生成单元测试/集成测试用例（JSON）的提示词。
- `prompts/vspec_script/script.md`：生成 Playwright 脚本的提示词。
- `prompts/vspec_testcase_reader/testcase_reader.html`：用于阅读 JSON 用例的单文件 HTML。
