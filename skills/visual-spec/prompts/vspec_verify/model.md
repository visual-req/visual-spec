...
6. 同时生成一份可视化 ER 图（SVG），写入：`/specs/models/er_diagram.svg`（必须）：
   ...
   - 排版要求：
     ...
     - 实体卡片之间保持足够间距，关系连线尽量避免相互交叉/重叠；连线不得穿过实体卡片文字
     - 每张实体卡片内：主键字段标注 `PK`，外键字段标注 `FK`，字段行左右对齐清晰
   - 生成后自检（必须）：
     - `/specs/models/er_diagram.svg` 必须存在且可单独打开
     - SVG 内的实体节点数量必须与 `/specs/models/*.md`（除 `README.md`）的实体数量一致，不得缺漏实体
     - SVG 中出现的每条关系必须能在对应实体文档的 `## 关系 / ## Relationships / ## リレーション` 中找到依据
     - 若实体或关系有遗漏/裁切/连线穿字：必须立即重绘并再次自检直到满足
