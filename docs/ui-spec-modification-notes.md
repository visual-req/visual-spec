...
因此引入统一且可编辑的 UI 规范文件：`/原型UI规范.md`。

## 变更点

- 新增约束文件：`/原型UI规范.md`（与 `/scheme.yaml` 同级）
- `/vspec:verify` 生成/更新原型时：
  - 若 `/原型UI规范.md` 不存在：先生成默认模板（不覆盖已存在文件）
  - 原型 UI 生成必须严格遵守 `/原型UI规范.md`
  - 若存在更严格的现有规范（例如 `/docs/current/ui_spec.md` 或 `/docs/current/ui_style.md`）：需把更严格的规则合并进 `/原型UI规范.md`，并以合并后的结果作为最终口径

## 如何修改 UI 规范

- 直接编辑目标项目根目录的 `/原型UI规范.md`
- 建议只改“口径”，不要写具体实现细节：
  - 色彩与状态映射
  - 表格/表单/详情的通用结构
  - Drawer/Modal 的使用规则
