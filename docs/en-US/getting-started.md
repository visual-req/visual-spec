...
- Initial spec: `/vspec:new`
  - Optional language:
    - Single language: `/vspec:new lang=zh` (or `lang=en`, `lang=ja`) sets `/scheme.yaml` `selected.language` and generates docs in that language
    - Multi-language (prototype switch): `/vspec:new lang=zh,en` sets docs default to `zh` and sets `/scheme.yaml` `selected.languages` for prototype UI language switching
