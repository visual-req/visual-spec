...
- 初期 spec：`/vspec:new`
  - 言語（任意）：
    - 単一言語：`/vspec:new lang=zh`（または `lang=en`、`lang=ja`）。`/scheme.yaml` `selected.language` を設定し、文書成果物をその言語で統一します
    - 多言語（プロトタイプ切替）：`/vspec:new lang=zh,en`。文書の既定言語を `zh` にし、`/scheme.yaml` `selected.languages` を切替用言語集合として設定します
